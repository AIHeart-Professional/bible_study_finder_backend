...
class GroupsDatabase:
    ...
    async def _resolve_user_id_from_public_id(self, public_id: str) -> Optional[int]:
        """Resolve user's public_id (UUID) to internal id (BIGINT)."""
        ...
        try:
            ...
            query = "SELECT id FROM users WHERE public_id = %s;"
            cursor.execute(query, (public_id,))
            result = cursor.fetchone()
            
            if result:
                user_id = int(result['id'])
                self.logger.debug(f"Resolved public_id {public_id} to internal id {user_id}")
                return user_id
            else:
                self.logger.warning(f"User with public_id {public_id} not found")
                return None
        except Exception as e:
            self.logger.error(f"Error resolving user_id from public_id: {e}", exc_info=True)
            return None
        finally:
            if connection:
                PostgresConnection.return_connection(connection)

    async def create_group(self, name: str, description: str, leaderPublicId: str, location: dict, image: Optional[str] = None) -> Optional[int]:
        ...
        try:
            connection = PostgresConnection.get_connection()
            cursor = connection.cursor()
            # public_id will be auto-generated by DEFAULT gen_random_uuid()
            query = """INSERT INTO groups (name, description, leader_user_id, location, image)
                       VALUES (%s, %s, %s, %s, %s) RETURNING id;"""
            cursor.execute(query, (name, description, leader_user_id, Json(location), image))
            group_id = cursor.fetchone()['id']
            connection.commit()
            self.logger.info(f"Group created successfully with ID: {group_id}")
            return int(group_id)
        except Exception as e:
    ...
    async def _resolve_user_public_id_from_user_id(self, user_id: int) -> Optional[str]:
        """Resolve user's internal id (BIGINT) to public_id (UUID)."""
        ...
        try:
            ...
            query = "SELECT public_id FROM users WHERE id = %s;"
            ...
            if result and result.get('public_id'):
                ...
                self.logger.warning(f"User with id {user_id} not found or has no public_id")
                return None
        except Exception as e:
    ...
    async def get_all_groups(self) -> List[dict]:
        ...
        try:
            connection = PostgresConnection.get_connection()
            cursor = connection.cursor()
            # Use JOIN to get leader public_id in one query
            query = """
                SELECT 
                    g.id,
                    g.public_id,
                    ...
                    g.created_at,
                    g.updated_at,
                    u.public_id as leader_public_id
                FROM groups g
                LEFT JOIN users u ON g.leader_user_id = u.id
